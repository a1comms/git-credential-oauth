export default {
    async fetch(request, env, ctx) {
        ...
        const bitBucketRegExp = new RegExp(/^\/bitbucket/);
        if (bitBucketRegExp.test(url.pathname)) {
            return fetchBitBucket(request, env, ctx);
        }

        const gitHubRegExp = new RegExp(/^\/github/);
        if (gitHubRegExp.test(url.pathname)) {
            return fetchGitHub(request, env, ctx);
        }

        return new Response("HTTP 404 Not Found", {
            status: 404,
...
async function fetchBitBucket(request, env, ctx) {
      ...
      <div class="login-box">
          <div class="card card-outline card-primary">
              <div class="card-header text-center">
                  <a href="#" class="h1"><b>BitBucket</b> for <b>Git</b></a>
              </div>
              <div class="card-body">
...
async function fetchGitHub(request, env, ctx) {
    const html = `<!DOCTYPE html>
        <html lang="en">
        ...
        <head>
            <meta charset="utf-8">
            <meta name="viewport" content="width=device-width, initial-scale=1">
            <title>GitHub App Installation for Git Authentication</title>
        ...
        <body class="hold-transition login-page">
            <div class="login-box">
                <div class="card card-outline card-primary">
                    <div class="card-header text-center">
                        <a href="#" class="h2"><b>GitHub App</b> Installation</b></a>
                    </div>
                    <div class="card-body">
                        <p class="login-box-msg">You have successfully installed the <b>GitHub App</b> required for authentication.</p>
                        <div class="row">
                            <div class="col-12">
                                <button type="submit" onclick="self.close()"
